...
### 部署记录（2026-08-05，用户选「我来手动启动」）

已做完（合并 `b8dcb88` 进 main → 迁移 → 清 `__pycache__`），**只差重启**：

- 生产工作树合并前在 main 且干净；合并后 `git status` 仍干净
  （`registry.json` / `config.json` 本来就 gitignore）。
- 迁移真跑：备份 `data/registry.json.bak-2026-08-05`，补 gid 120 个、
  关键词升级 17 条、从 config 合并 1 条，无悬空关键词。
- 清掉了 `plugins/**/__pycache__`（SMB 直编辑坑，CLAUDE.md 3.6）。
- ★★ **踩到并已规避：改数据格式的部署有个"旧进程还在跑"的断档窗口**。
  迁移把 `invite_keywords` 升级成 dict 之后，**还没重启的旧进程会当场
  `TypeError: unhashable type: 'dict'`**（旧 `invite.py` 直接拿值当群名去寻址），
  也就是从迁移完到人重启之间**拉群是坏的**——而"人手动重启"意味着这段时间不可控。
  处置：迁移后把 `invite_keywords` **临时降回纯字符串**（gid 保留，旧代码不认识
  这个字段也不会碰），实测旧代码/新代码吃同一份数据结果完全一致（关键词 18 条、
  转发目标 105 个、寻址候选逐项相同）。重启后再跑一次迁移即可重新升级（幂等）；
  即便忘了跑也不影响功能——新代码本来就兼容旧格式。
  **教训：任何"改磁盘数据格式"的上线，都要先问一句"没重启的老进程读得动吗"。
  读不动就把格式升级推迟到重启之后，别赌那段窗口没人用。**

### 判据 4 的预演（2026-08-05，拿当前生产数据的副本跑，没碰真环境）

真机那一下（微信 UI）必须等重启后由人在真微信里发消息，但**除此之外的整条链路
已经用生产数据演练通过**：

1. 面板 `invite.save` 新增「验收暗号」→ NCC的大理朋友们3群 → 提示"下一条私聊消息即生效"；
2. 假装有人私聊发这个词 → `invite.handle_invite` 命中 → **按🐶备注
   「NCC的大理朋友们3群🐶」切窗口** → `AddGroupMembers(['路人甲'])` → 成功时静默不回话；
3. 面板上把它**停用** → 再发一次不命中、连窗口都不切（`enabled=False` 生效）；
4. 面板上删掉 → 关键词回到 18 条、转发目标 105 个，与部署前一致。

也就是说"面板改了→机器人立刻按新配置干活"这件事本身已经验过，
真机验收要确认的只剩微信 UI 那一下真的把人加进群。

### 还剩（§0 判据 4 的真机部分）—— 需要先重启
