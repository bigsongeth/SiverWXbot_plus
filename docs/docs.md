...
**环境要求：**
- Windows 操作系统
- Windows wx PC 版（`4.1.9` ~ `4.1.12.55` 版本）
...
独立分支版本的管理面板内置了一位专属 **AI 面板管家**。它运行在你自己的 Windows 电脑上，可以通过**自然语言对话**的方式，帮你完成面板配置、Prompt 编写、运行状态检查、故障排查，以及源码版的功能定制开发。不用翻文档、找字段、手改 JSON，像聊天一样把需求告诉它即可。
...
- 它可以直接读取面板的配置、Prompt、记忆和日志，理解你的需求后**替你完成操作**，并在修改后提醒你重新载入配置。
...
| 添加定时消息 | "添加一个每天早上 9 点发送到『XXX群』的定时消息" |
| 优化 Prompt | "帮我优化默认 Prompt，让回复语气更活泼" |
| 检查配置 | "检查一下我现在的模型配置有没有问题" |
...
| 优势 | 说明 |
|------|------|
| 快速配置 | 一句话完成原本需要翻多个页面、手改 JSON 的配置工作 |
...
4. 关闭wx自动更新。
5. 适配 Windows wx `4.1.9` - `4.1.12.55`。
