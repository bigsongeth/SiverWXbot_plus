...
**根因仍未知**，已把现象整理成一段话发社区问了。

#### 2026-09-02 复盘：1154 个探针样本 + 8 月全部日志跑出来的结论（别再重走）

- **探针和自愈 08-15 起已关**（`data/config.json` 里 `probe.enabled=false`）：
  08-11~08-15 自愈重启了 20 次，**重启治不了**——08-12 每次重启后第 1 次探针成功、
  第 2 次起继续失败，一上午重启 5 次全是这个节奏。「进坏状态→重启就好」这个模型是错的。
- **失败 = 微信压根没建出新窗口**（`win_before == win_after`，前后两份顶层窗口清单都没有
  靶子），不是"建了没找到"。wxautox 拿到的 hwnd 是 0 才报 `MoveWindow 1400`。
- 又证伪了四个假说（原来的三个见上）：
  ④ 前台是谁 / 会话列表被别的窗口盖住：`hit_test` 显示主窗口就在最上面的样本里
    OK 70 次、BAD 16 次，不区分；
  ⑤ 别的线程并发操作 UI：78 次失败里只有 1 次（08-14 20:04「检查群组 全部」跑着）；
  ⑥ wxautox 走「找到当前会话」分支：该分支 OK 1123 / BAD 367，「切换」分支 OK 55 / BAD 42；
  ⑦ RustDesk 连着：33 次失败时 `rustdesk_conns=0`。
- **唯一还活着的相关项**：主窗口 rect 是 `1500x1048/1056` 时失败率 31%（17/54），
  `1040x736` 时 2%（3/128）；以及凌晨 01–07 点 335 个样本零失败（那会儿没人、没消息）。
  两者都指向"负载/时序"而不是"状态"。
- ★ **41.x 的双击不是真鼠标**：`uiautomation.Control.DoubleClick(move=False)` 默认走
  `uiplug.Win32.click_by_bbox(bbox, double_click=True, activate=False)`，往句柄**发消息**。
  所以鼠标在哪、谁在前台都不相关——而"发了什么消息、发给哪个 hwnd、两下间隔多久"
  **从来没人看过**。`uiplug.pyd` 是编译的，但它在调用时才从 `win32gui`/`win32api`/
  `ctypes.windll.user32` 取属性，**打桩这些模块就能把它看透**。
- 实验工具 `plugins/listen_health/diag_open_window.py`（会话 2、机器人停掉后跑，见文件头）：
  给底层点击调用打桩、循环 N 次开关靶子窗口、失败截屏，`--load K` / `--resize WxH` 做 A/B。
  带登录态守门：主窗口不在登录态立刻停，别对着登录界面连戳。

**2026-09-03 凌晨实跑结果（独立进程，机器人停掉）**：空载 8/8、CPU 打满 86–100% 8/8、
主窗口 1500×1048 8/8，**24/24 全成功，一次都复现不出来**。于是又死两个假说：
⑧ CPU 负载拖慢双击时序；⑨ 主窗口尺寸。录到的点击序列每次都一样：
`win32api.SendMessage` 往主窗口句柄发 按下/抬起、按下/抬起、`WM_LBUTTONDBLCLK` 共 5 条，
客户区坐标固定在会话项上，整个序列 0.5–1.0 秒。**结论：故障只在生产上下文里出现**
（进程跑了很久、开着 4–5 个子窗口、刚扫到新私聊立刻开窗），拿出来单测是测不到的。
- 所以现在挂了 **`tap.py` 开窗录像机**（`probe.register` 里装，不依赖探针开关，
  配置 `tap` 段）：常驻在 bot 进程里包住 `wx.AddListenChat`，执行期间录底层点击消息，
  失败时连窗口清单、前台、hit_test、截图一起存 `data/tap-YYYYMMDD.jsonl` +
  `data/tap-shot-*.png`，成功也记一行（只有调用序列）用来对比。
  **下次再报 MoveWindow 1400，直接看那一行的 `calls`**：一条点击都没有 → 点击走了没打桩的通道；
  坐标/hwnd 和成功行一样 → 微信收到了同样的消息却没建窗，问题在微信侧；不一样 → wxautox 找错了
  会话项。注意它装在 `init_wx_listeners` 的注册处，**初始化那 5 个监听发生在它之前、录不到**，
  只录动态监听；要录初始化就得把 hook 往前挪。单测 `tests/test_listen_tap.py`（9 个）。
- ★ **微信「为了你的账号安全，请重新登录」是这台机器的常客，别一出事就往自己头上揽**：
  从 wxautox 日志里微信 PID 变化推出的强制重登史：08-13 00:05、08-13/14 之间、08-17 23:57、
  09-02 20:52、09-03 00:08（这次撞在实验第 1 轮之后 6 秒，但随后 24 轮同样操作一次没踢）。
  三次落在零点前后。重登后 wxautox 缓存的 HWND 失效，机器人要走完整初始化（点头像读资料）。
- ★ **开始菜单开着会让 wxautox 完整初始化失败**（09-03 00:23 实测）：报
  `COMError 事件无法调用任何订户` 或 `'ProfileWnd' object has no attribute 'topui'`，
  微信本身好好的。前台是 `SearchApp.exe` 就是这个情况，发一个 Esc 关掉再启动。
- `restart_panel.bat` 起来之后进程列表里会有**两个** `web_server.py`（一个 `python web_server.py`、
  一个全路径的，只有后者监听 10001），重启前后都是这个结构，不是残留。

#### ★★★ 2026-09-08 真·根因：真实鼠标指针停在微信窗口顶边，假双击就永远不认（可随意开关复现）

09-08 00:27 机器人启动后，前 3 个监听成功、后 2 个（美食地图/📈）+ 新私聊 Pablo 共 24 次双击全败，
12 次「真实单击复位」全部无效。录像机对比后发现：**成功的 3 个走的是搜索框→右键菜单「在独立窗口打开」
路径（会话不在可见列表里时 wxautox 才走），压根没用双击**；失败的全是列表里直接假双击。
所以"前面好后面坏"是假象，微信从启动那一刻起就在"双击不认"状态里，只是有条路不用双击。

用 `diag_open_window.py --cursor X,Y`（只移真实指针、不点击）做了 16 轮 A/B，想坏就坏想好就好：

| 真实指针停在 | 结果 |
|---|---|
| 主窗口顶边 (1098,0)、(1098,2)、(300,0) | **0/6**，DBLCLK 处理 0.03–0.05s（没建窗） |
| 子窗口顶边 (700,2) | **0/2** |
| 主窗口内部 y=10/30/60/90、(700,600) | 10/10 |
| 子窗口内部 (700,600)、Chrome 上、任务栏上 | 6/6 |

机制：指针压在窗口最顶上那几个像素（拖拽/调整大小的边框带，HTTOP）时，微信对假双击的解读就变了；
单击照常生效（截图里会话被选中），只是第二下不算双击。Qt 内部具体哪一行没追到底，
但现象是确定性的、与线程数/负载/窗口尺寸/谁在前台都无关。
**RustDesk 会把远端指针留在屏幕顶边**（09-08 00:46 实测停在 (1098,0)），这解释了此前所有相关性：
白天有人连过就坏、凌晨没人不坏；重启后第一个成功（走搜索菜单）其余全败；主窗口越宽指针越容易压在它顶边；
真实单击"有时能救"= 09-06 诊断把指针留在了窗口里，"有时不能"= 生产 unstick 点完把指针放回顶边。
**09-06 那套「监听线程交错→Qt 按钮状态卡住」的结论很可能被这个混淆了**（实验期间有人连着 RustDesk 在动鼠标）；
`LISTENER_EXCUTOR_WORKERS=1` 无害，保留。

修法（`plugins/listen_health/tap.py`，2026-09-08 上线）：
- `guard_cursor`：每次 `AddListenChat` 前看真实指针，压在任一可见微信窗口的边框带（离任一条边 ≤ `tap.cursor_band`=8px）
  上就 `SetCursorPos` 挪到主窗口内部安全点（只移动不点击），日志「真实指针停在「xx」边框带上 … 已挪到」。
  指针在别的程序上（Chrome/任务栏）不碰，人正在用鼠标时不打扰。开关 `tap.cursor_guard`。
- `unstick` 点完**不再把指针放回原处**。
- 探针/录像机快照新增 `cursor` / `cursor_win`，以后每条失败记录都带指针位置。
- 生产验证：指针停在 (700,600) 启动 → 5 个初始监听全部一次成功（01:06:56 校验通过）。
- 单测 `tests/test_listen_tap.py`（29 个，含 `cursor_danger` / 守卫顺序 / 失败不阻断）。
- 一条副作用提醒：mac 上跑 `tests/test_listen_health.py` 会往生产 `panel_logs/log_*.txt` 写几行
  「自启动标记已写入 /var/folders/...」（ui_watchdog 的 `log()` 直连面板日志），看到别当成生产事件。

<details><summary>2026-09-06 的结论（已被上面推翻，留作考古）</summary>
